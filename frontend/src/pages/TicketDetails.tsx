...
export function TicketDetails() {
    ...
    return (
        <div className="space-y-6 pb-32 animate-fade-in font-sans max-w-[1600px] mx-auto p-4 md:p-6">
            ...
            <div className="md:col-span-9">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-[300px]">
                    ...
                    {ticket.snapshot_url ? (
                        <>
                        <img src={getMediaUrl(ticket.snapshot_url)} alt="Foto LPR da passagem do veículo" className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
                ...
                <div className={`bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 ${!isFinalizado ? 'mt-2' : 'h-full'}`}>
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-thin">
                        ...
                        <div key={idx} className="relative group w-20 h-20 flex-shrink-0 bg-slate-100 rounded-lg overflow-hidden cursor-pointer hover:ring-2 ring-blue-500 transition border border-slate-200" onClick={() => setMediaModal({url, type: 'image'})}>
                            <img src={getMediaUrl(url)} alt="Foto de evidência" className="w-full h-full object-cover"/>
                            <button type="button" title="Deletar foto" onClick={(e) => handleDeleteFoto(url, e)} className="absolute top-1 right-1 bg-red-600 hover:bg-red-700 text-white p-1 rounded-md opacity-0 group-hover:opacity-100 transition-all shadow-sm"><Trash2 size={10}/></button>
                        </div>
                        ))}
                    </div>
                    ) : (
            ...
            {/* FOOTER DE AÇÕES */}
            <div className="flex justify-between items-center no-print sticky bottom-0 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-md p-4 md:p-6">
