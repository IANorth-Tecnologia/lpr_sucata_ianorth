...
import { Sun, Moon, Truck, LayoutDashboard, Menu, X, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    <div className="flex h-screen w-full bg-light-bg dark:bg-dark-bg text-slate-900 dark:text-slate-100 font-sans transition-colors duration-300">
      ...
      <aside 
        className={`
            ...
            flex flex-col shadow-lg z-20 transition-all duration-300 ease-in-out
            ${isSidebarOpen ? 'w-64 translate-x-0' : 'w-0 -translate-x-full opacity-0 overflow-hidden md:w-0 md:opacity-100 md:translate-x-0'}
        `}
      >
        ...
        <div className="p-4 border-t border-light-border dark:border-dark-border text-center">
            <p className="text-xs text-slate-400 dark:text-slate-500 font-mono">v2.1.0 • IAnorth</p>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="h-16 bg-light-surface dark:bg-dark-surface border-b border-light-border dark:border-dark-border flex items-center justify-between px-6 shadow-sm z-10">
            ...
            <div className="flex items-center gap-4">
                <span className="text-xs font-bold px-2 py-1 bg-green-100 text-green-700 rounded-full border border-green-200 animate-pulse">
                    ● Online
                </span>
                <button 
                    onClick={toggleTheme}
                    className="p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors text-slate-600 dark:text-yellow-400"
                >
